...
# Set leverage and margin mode CROSS
...
def has_open_positions():
    try:
        for sym in SYMBOLS:
            ...
            if pos and abs(float(pos[0].get('positionAmt') or 0))>1e-8: return True
        for kc_sym in KUCOIN_TRADE_SYMBOLS:
            ...
            if pos:
                ...
                try:
                    ...
                    raw = float(info.get('currentQty') or 0)
                except: pass
                if abs(raw)>1e-8: return True
        return False
    except: return True

# FIXED close_all_and_wait() — ONLY CHANGE IN ENTIRE SCRIPT
def close_all_and_wait(timeout_s=20, poll_interval=0.5):
    ...
    # === BINANCE CLOSE — FINALLY FIXED (short → buy, long → sell) ===
    for sym in SYMBOLS:
        try:
            ...
            close_side = "sell" if raw_amt > 0 else "buy"   # CORRECT LOGIC
            qty = abs(raw_amt)
            ...
            print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Closing Binance {sym} {close_side.upper()} {qty} (positionAmt={raw_amt})")
            ...
            print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Binance reduceOnly {close_side.upper()} {qty} → SUCCESS")

        except Exception as e:
            print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Binance close failed (will retry next loop): {e}")

    # === KUCOIN CLOSE — UNCHANGED & PERFECT ===
    all_kc_positions = []
    try:
        all_kc_positions = kucoin.fetch_positions(symbols=KUCOIN_TRADE_SYMBOLS)
    except Exception as e:
        print(f"Error fetching KuCoin positions: {e}")
        ...
        for pos in all_kc_positions:
            ...
            if qty > 0:
                ...
                try:
                    ...
                    print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} KUCOIN close failed: {e}")
    ...
    closing_in_progress = False
    print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Timeout waiting for close.")
    print("="*72)
    return False
...
def extract_executed_price_and_time(exchange, symbol, order_obj):
        ...
        if trades:
            ...
            if px:
                if ts:
                    try:
                        ts_ms = int(ts)
                        if ts_ms < 1e12 and ts_ms > 1e9: ts_ms = ts_ms*1000
                        iso = datetime.utcfromtimestamp(ts_ms/1000.0).isoformat()+'Z'
                        return float(px), iso
                    except: pass
    ...
    try:
        ...
        if mid: return float(mid), datetime.utcnow().isoformat()+'Z'
    except Exception: pass
    return None, None
...
def safe_create_order(exchange,side,notional,price,symbol, trigger_time=None, trigger_price=None):
    ...
    try:
        ...
        slippage = exec_price - float(trigger_price) if trigger_price is not None and exec_price is not None else None
        latency_ms = None
...
def match_base_exposure_per_exchange(bin_exchange, kc_exchange, bin_symbol, kc_symbol, desired_usdt, bin_price, kc_price):
    ...
    bin_prec = None; kc_prec = None; bin_contract_size = 1.0; kc_contract_size = 1.0
    try:
        if m_bin:
    ...
    kc_contracts = round_down(target_base / kc_contract_size, kc_prec) if kc_prec is not None and kc_contract_size > 0 else target_base
    ...
    if bin_base_amount <= 0:
        ...
        bin_base_amount = round_down(step_bin / bin_price, bin_prec) if bin_prec is not None else step_bin / bin_price
        notional_bin = bin_base_amount * float(bin_price)
    if kc_contracts <= 0:
        step_kc = (kc_contract_size * kc_price) if kc_contract_size and kc_price else (desired_usdt * 0.001)
        kc_contracts = round_down((step_kc / kc_contract_size) if kc_contract_size else step_kc, kc_prec) if kc_prec is not None else (step_kc / kc_contract_size if kc_contract_size else step_kc)
        notional_kc = kc_contracts * float(kc_contract_size) * float(kc_price)
    return float(notional_bin), float(notional_kc), float(bin_base_amount), float(kc_contracts)

def get_prices():
    bin_prices = {}; kc_prices = {}
    try:
...
while True:
    try:
        ...
        for i,sym in enumerate(SYMBOLS):
            ...
            if bin_ask >= kc_bid and bin_bid <= kc_ask:
                entry_confirm_count[sym] = 0
            ...
            if bin_ask < kc_bid:
                ...
                if positions[sym] is None and trigger_spread >= ENTRY_SPREAD and not closing_in_progress and not has_open_positions():
                    ...
                    if entry_confirm_count[sym] >= 3:
                        ...
                        if ok_kc and ok_bin and exec_price_kc and exec_price_bin:
                            ...
                            entry_confirm_count[sym] = 0
                            print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} ENTRY CASE A SUCCESS | Captured basis: {final_entry_spread:.3f}%")
                        else:
                            print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Partial entry → closing")
                            close_all_and_wait()
                            entry_confirm_count[sym] = 0

            # CASE B — FULLY FIXED (KuCoin denominator + same captured formula)
            elif bin_bid > kc_ask:
                ...
                if positions[sym] is None and trigger_spread >= ENTRY_SPREAD and not closing_in_progress and not has_open_positions():
                    ...
                    if entry_confirm_count[sym] >= 3:
                        ...
                        if ok_kc and ok_bin and exec_price_kc and exec_price_bin:
                            ...
                            trade_start_balances[sym]=start_total_balance
                            entry_confirm_count[sym] = 0
                            print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} ENTRY CASE B SUCCESS | Captured basis: {final_entry_spread:.3f}%")
                        else:
                            print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Partial entry → closing")
                            close_all_and_wait()
                            entry_confirm_count[sym] = 0

            # EXIT LOGIC — SAME CAPTURED FORMULA FOR BOTH CASES
            if positions[sym] is not None:
                ...
                captured = entry_spreads[sym] - current_exit_spread   # EXACT SAME FORMULA BOTH SIDES

                current_entry_spread = 100*(kc_bid - bin_ask)/bin_ask if bin_ask < kc_bid else 100*(bin_bid - kc_ask)/kc_ask if bin_bid > kc_ask else 0

                print(f"{datetime.now().strftime('%H:%M:%S')} POSITION OPEN | Live Spread: {current_entry_spread:.3f}% | Entry Basis: {entry_spreads[sym]:.3f}% | Exit Spread: {current_exit_spread:.3f}% | Captured: {captured:.3f}% | Exit Confirm: {exit_confirm_count[sym] + (1 if (captured >= PROFIT_TARGET or abs(current_exit_spread)<0.02) else 0)}/3")
                
                if captured >= PROFIT_TARGET or abs(current_exit_spread) < 0.02:
                    exit_confirm_count[sym] += 1
                    if exit_confirm_count[sym] >= 3:
                        print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} EXIT 3/3 | Captured {captured:.3f}% → CLOSING")
                        close_all_and_wait()
                        positions[sym] = None
