...
# Fully corrected copy-paste version with explicit exit-close logic for Case B (closes Binance by going LONG)
...
# --- NEW helper: Get Total Futures Balance in USDT ---
            ...
            if positions_bin:
                ...
                if abs(raw) > 0:
                    ...
                    if qty > 0:
                        ...
                        try:
                            # Use reduceOnly param to avoid increasing position
                            try:
                                binance.create_market_order(sym, side, qty, params={'reduceOnly': True})
                            except Exception:
                                binance.create_order(sym, 'market', side, qty, None, {'reduceOnly': True})
                        except Exception as e:
...
def safe_create_order(exchange,side,notional,price,symbol, trigger_time=None, trigger_price=None):
    ...
    try:
        ...
        if exchange.id=='binance':
            # use create_market_order for binance (no params) — used for entry; for explicit closes we use direct reduce-only path elsewhere
            if side.lower()=='buy':
...
# Quick helper to submit a reduce-only market order on an exchange to close a position leg
def quick_close_leg(exchange, symbol, raw_position_amount):
    """
    exchange: ccxt exchange object
    symbol: trading symbol
    raw_position_amount: signed position amount (positive long, negative short) as float
    Returns True if order sent successfully (best-effort), False otherwise.
    """
    try:
        if abs(float(raw_position_amount)) <= 0:
        ...
        qty = abs(float(raw_position_amount))
        qty = round_down(qty, prec) if prec is not None else qty
        if qty <= 0:
            # fallback minimal qty attempt using contractSize and ticker
            try:
                contract_size = float(market.get('contractSize') or market.get('info', {}).get('contractSize') or 1.0)
            except:
                contract_size = 1.0
            try:
                t = exchange.fetch_ticker(symbol)
                est_price = (t.get('bid') or t.get('last') or (t.get('info') or {}).get('lastPrice')) or 1.0
            except:
                est_price = 1.0
            minimal_qty = max(1e-6, (contract_size / (est_price or 1.0)))
            qty = round_down(minimal_qty, prec) if prec is not None else minimal_qty
            if qty <= 0:
                return False

        # determine close side: if raw>0 (long) we need to sell to close; if raw<0 (short) buy to close
        side = 'sell' if float(raw_position_amount) > 0 else 'buy'

        # Use reduceOnly param for safety
        try:
            # For Binance futures, create_market_order doesn't accept params reliably on some CCXT versions; try both
            if exchange.id == 'binance':
                try:
                    exchange.create_market_order(symbol, side, qty, params={'reduceOnly': True})
                except Exception:
                    exchange.create_order(symbol, 'market', side, qty, None, {'reduceOnly': True})
            else:
                exchange.create_market_order(symbol, side, qty, params={'reduceOnly': True, 'marginMode': 'cross'})
        except Exception as e:
            print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} quick_close_leg error for {exchange.id} {symbol}: {e}")
            return False

        return True
    except Exception as e:
        print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} quick_close_leg unexpected error: {e}")
        return False
        ...
        for i,sym in enumerate(SYMBOLS):
            ...
            if positions[sym] is not None:
                if positions[sym]=='caseA':
                    ...
                    # CORRECT captured calculation: EntrySpread - CurrentExitSpread
                    captured = entry_spreads[sym] - current_exit_spread
                ...
                if exit_condition:
                    ...
                    if exit_confirm_count[sym] >= 3:
                        ...
                        # ---------- Explicit quick parallel close (preferred) ----------
                        # Decide closing sides based on positions[sym]
                        if positions[sym] == 'caseA':
                            # Entry was: buy Binance (long), sell KuCoin (short)
                            # To close: sell on Binance, buy on KuCoin
                            bin_close_side = 'sell'
                            kc_close_side  = 'buy'
                        else:
                            # caseB: entry was sell Binance (short), buy KuCoin (long)
                            # To close: buy on Binance (close short), sell on KuCoin
                            bin_close_side = 'buy'
                            kc_close_side  = 'sell'

                        # Fetch raw position amounts to compute quick close qtys
                        # For Binance
                        bin_raw = 0.0
                        try:
                            posb = binance.fetch_positions([sym]) or []
                            if posb:
                                # try multiple fields
                                cand = posb[0].get('positionAmt') or posb[0].get('contracts') or posb[0].get('size') or (posb[0].get('info') or {}).get('positionAmt')
                                bin_raw = float(cand or 0)
                        except Exception as e:
                            print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Error fetching binance pos for quick close: {e}")

                        # For KuCoin
                        kc_raw = 0.0
                        try:
                            posk = kucoin.fetch_positions([kc_trade_sym]) or []
                            if posk and len(posk)>0:
                                info = posk[0].get('info') or {}
                                cand = info.get('currentQty') or posk[0].get('contracts') or posk[0].get('positionAmt')
                                kc_raw = float(cand or 0)
                        except Exception as e:
                            print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Error fetching kucoin pos for quick close: {e}")

                        # Fire quick closes in parallel (non-blocking threads) using quick_close_leg
                        results_close = {'bin': False, 'kc': False}
                        def do_close_bin():
                            try:
                                ok = quick_close_leg(binance, sym, bin_raw)
                                results_close['bin'] = ok
                            except Exception as e:
                                print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} quick close bin exception: {e}")
                                results_close['bin'] = False

                        def do_close_kc():
                            try:
                                ok = quick_close_leg(kucoin, kc_trade_sym, kc_raw)
                                results_close['kc'] = ok
                            except Exception as e:
                                print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} quick close kc exception: {e}")
                                results_close['kc'] = False

                        tcb = threading.Thread(target=do_close_bin)
                        tck = threading.Thread(target=do_close_kc)
                        tcb.start(); tck.start(); tcb.join(); tck.join()

                        if results_close.get('bin') and results_close.get('kc'):
                            print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Quick parallel close succeeded for {sym}")
                            positions[sym] = None
                            exit_confirm_count[sym] = 0
                        else:
                            print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Quick parallel close partial/failure (bin_ok={results_close.get('bin')} kc_ok={results_close.get('kc')}). Falling back to robust close_all_and_wait().")
                            close_all_and_wait()
                            positions[sym] = None
                            exit_confirm_count[sym] = 0
