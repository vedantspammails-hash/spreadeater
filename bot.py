    ...
    for ex in [binance, kucoin]:
        try:
            ...
            try:
                ...
                print(f"Market load failed on {getattr(ex, 'id', 'unknown')}: {e}")
                pass
        ...
        for i,sym in enumerate(SYMBOLS):
            ...
            if positions[sym] is not None:
                if positions[sym]=='caseA':
                    ...
                    # FIXED: compute current_exit_spread in same relative basis as entry and compute captured similarly to Case A
                    current_exit_spread = 100*(bin_bid - kc_ask)/entry_prices[sym]['kucoin']
                    # Previously used current_exit_spread - entry_spreads which had wrong sign; change to entry_spreads - current_exit_spread
                    captured = entry_spreads[sym] - current_exit_spread
                ...
                if exit_condition:
                    ...
                    if exit_confirm_count[sym] >= 3:
                        ...
                        # --- SPECIAL: If Case B, try a Binance-specific direct-close attempt before generic close_all_and_wait ---
                        if positions[sym] == 'caseB':
                            print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} CASE B -> Attempting Binance direct-close (closePosition=True) BEFORE generic close_all_and_wait()")
                            try:
                                # Try to fetch position info and log it
                                try:
                                    pos_info = binance.fetch_positions([sym])
                                    print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Binance fetch_positions returned: {pos_info}")
                                except Exception as e:
                                    pos_info = None
                                    print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Binance fetch_positions failed: {e}")

                                # First attempt: try close via Binance-specific 'closePosition' param (no qty computation)
                                tried_closeposition = False
                                cp_result = None
                                try:
                                    # Many ccxt/binance wrappers accept params {'reduceOnly': True, 'closePosition': True}
                                    # We attempt to create a market order with closePosition param and amount=None.
                                    # Some ccxt versions require 'create_order' with amount=0 or None; we'll try None first.
                                    print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Attempting binance.create_order with closePosition=True (amount omitted)")
                                    try:
                                        cp_result = binance.create_order(sym, 'market', 'buy' if True else 'sell', None, None, params={'reduceOnly': True, 'closePosition': True})
                                    except TypeError:
                                        # Some ccxt wrappers require positional args; fallback to create_market_order with params and amount=0
                                        cp_result = binance.create_order(sym, 'market', 'buy' if True else 'sell', 0, None, params={'reduceOnly': True, 'closePosition': True})
                                    print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Binance closePosition attempt response: {cp_result}")
                                    tried_closeposition = True
                                except Exception as e:
                                    print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} binance closePosition attempt raised: {e}")
                                    tried_closeposition = True
                                    cp_result = None

                                # If closePosition attempt didn't work or didn't close, fallback to fetching actual open qty and closing reduceOnly by qty (robust, logged)
                                closed_ok = False
                                if cp_result:
                                    # try to determine if order closed the position by checking returned object and balances/positions
                                    print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} closePosition response logged, verifying if position closed by querying positions...")
                                    try:
                                        time.sleep(0.5)
                                        post_pos = binance.fetch_positions([sym])
                                        print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Binance positions after closePosition attempt: {post_pos}")
                                        raw_after = float(post_pos[0].get('positionAmt') or post_pos[0].get('contracts') or 0)
                                        if abs(raw_after) == 0:
                                            print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Binance position appears closed after closePosition attempt.")
                                            closed_ok = True
                                        else:
                                            print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Binance position still open after closePosition attempt: positionAmt/size = {raw_after}")
                                    except Exception as e:
                                        print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Could not verify position after closePosition attempt: {e}")

                                if not closed_ok:
                                    # Fallback: compute qty robustly and submit reduceOnly market order (this is what was used previously, but heavily logged)
                                    print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Falling back to robust qty calculation for Binance close (logged).")
                                    try:
                                        pos_before = binance.fetch_positions([sym])
                                        print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Binance fetch_positions returned for fallback: {pos_before}")
                                        raw_qty = 0.0
                                        if pos_before:
                                            raw_qty = float(pos_before[0].get('positionAmt') or pos_before[0].get('contracts') or 0)
                                        # If still zero, try reading nested info
                                        if abs(raw_qty) == 0 and pos_before and isinstance(pos_before[0].get('info'), dict):
                                            info = pos_before[0].get('info') or {}
                                            raw_qty = float(info.get('positionAmt') or info.get('positionAmtVal') or info.get('currentQty') or 0)
                                        print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Determined Binance raw_qty for fallback: {raw_qty}")
                                        if abs(raw_qty) > 0:
                                            side = 'sell' if raw_qty > 0 else 'buy'
                                            market = get_market(binance, sym)
                                            prec = market.get('precision',{}).get('amount') if market else None
                                            qty = round_down(abs(raw_qty), prec) if prec is not None else abs(raw_qty)
                                            print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Submitting fallback Binance reduceOnly market order to close: {side} {qty} (precision={prec})")
                                            try:
                                                ord_resp = binance.create_market_order(sym, side, qty, params={'reduceOnly': True})
                                                print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Fallback Binance close order response: {ord_resp}")
                                            except Exception as e:
                                                print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Fallback Binance close order failed: {e}")
                                        else:
                                            print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Fallback: could not determine non-zero raw_qty for Binance; skipping direct-close attempt.")
                                    except Exception as e:
                                        print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Exception during Binance fallback close attempt: {e}")

                        # Now run the generic close routine (KuCoin close is handled there)
                        close_all_and_wait()
                        positions[sym]=None
