...
def has_open_positions():
    try:
        ...
        for kc_sym in KUCOIN_TRADE_SYMBOLS:
            ...
            if pos:
                raw = 0
                try: raw = float(pos[0].get('contracts') or pos[0].get('positionAmt') or 0)
                except: pass
                if raw == 0 and isinstance(pos[0].get('info'), dict):
                    raw = float(pos[0]['info'].get('currentQty',0))
                if abs(raw)>0: return True
        return False
    except: return True

# FIXED close_all_and_wait – Binance uses closePosition=True (the only change!)
def close_all_and_wait(timeout_s=20, poll_interval=0.5):
    ...
    # === BINANCE: Use closePosition (no qty needed – 100% safe) ===
    for sym in SYMBOLS:
        try:
            pos_info = binance.fetch_positions([sym])[0]
            amt = float(pos_info.get('positionAmt') or 0)
            if abs(amt) < 1e-8:
                continue
            side = 'buy' if amt < 0 else 'sell'
            print(f"{datetime.now().strftime('%H:%M:%S.%f')[:-3]} Closing Binance {sym} FULL POSITION (closePosition)")
            binance.create_market_order(sym, side, None, params={'closePosition': True})
        except Exception as e:
            print(f"Binance close failed: {e}")

    # === KUCOIN: Keep original reduceOnly (already works perfectly) ===
    all_kc_positions = []
    try:
    ...
    for pos in all_kc_positions:
        ...
        if qty > 0:
            ...
            try:
                ...
                print(f"KUCOIN close order failed: {e}")
...
# Rest of your functions (extract_executed_price_and_time, safe_create_order, match_base_exposure_per_exchange, get_prices, etc.)
# ... unchanged – everything below is exactly as you had it ...
                ...
                if positions[sym] is None and trigger_spread >= ENTRY_SPREAD and not closing_in_progress and not has_open_positions():
                    ...
                    if entry_confirm_count[sym] >= 3:
                        ...
                        if ok_kc and ok_bin and exec_price_kc is not None and exec_price_bin is not None:
                            ...
                            print(f" KuCoin executed: price={exec_price_kc} exec_time={exec_time_kc}")
                            print(f" Binance executed: price={exec_price_bin} exec_time={exec_time_bin}")
