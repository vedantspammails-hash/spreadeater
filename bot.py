...
import random  # <-- needed for speed jitter
...
PROFIT_TARGET = 0.4
MARGIN_BUFFER = 1.02
LIQUIDATION_GRACE_SECONDS = 10.0  # OPTIONAL CONFIG LINE
...
# --- LIQUIDATION PROTECTION STATE (NEW) ---
...
while True:
    try:
        ...
        for i,sym in enumerate(SYMBOLS):
            ...
            # --- LIQUIDATION PROTECTION CHECK (NEW) ---
            if positions[sym] is not None and not closing_in_progress:
                since = time.time() - entry_timestamp.get(sym, 0)
                if since < LIQUIDATION_GRACE_SECONDS:  # 10-second grace period (configurable)
                    pass
                ...
                if positions[sym] is None and trigger_spread >= ENTRY_SPREAD and not closing_in_progress and not has_open_positions():
                    ...
                    if entry_confirm_count[sym] >= 3:
                        ...
                        if ok_kc and ok_bin and exec_price_kc is not None and exec_price_bin is not None:
                            ...
                            positions[sym]='caseA'
                            entry_timestamp[sym] = time.time()  # --- NEW: record entry time for liq protection
